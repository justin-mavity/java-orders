package com.lanbdaschool.orders.services;
...
import com.lanbdaschool.orders.repositories.AgentsRepository;
import com.lanbdaschool.orders.repositories.CustomersRepository;
import com.lanbdaschool.orders.repositories.OrdersRepository;
import com.lanbdaschool.orders.repositories.PaymentRepository;
...
public class SeedData implements CommandLineRunner {
    ...
    public void run(String[] args) throws Exception
    {
        ...
        for (String theName : customerNames) {
            ...
            String tempWorkingarea = dataFaker.address().cityName();
            String tempCustcountry = dataFaker.address().country();
            String tempGrade = dataFaker.country().countryCode2();
            double tempOpeningamt = dataFaker.number().randomDouble(2, 0, 10000);
            double tempReceiveamt = dataFaker.number().randomDouble(2, 0, 10000);
            double tempPaymentamt = dataFaker.number().randomDouble(2, 0, 10000);
            double tempOutstandingamt = dataFaker.number().randomDouble(2, 0, 10000);
            String tempPhone = dataFaker.phoneNumber().phoneNumber();

            Customer fakeCustomer = new Customer(theName, custcity, tempWorkingarea, tempCustcountry, tempGrade, tempOpeningamt, tempReceiveamt, tempPaymentamt, tempOutstandingamt, tempPhone, a10);

            int randomNumber = random.nextInt(10); // random number 0 through 9
            for (int i = 0; i < randomNumber; i++) {
                double tempGetOrdamount = dataFaker.number().randomDouble(2, 0, 10000);
                double tempGetAdvanceamount = dataFaker.number().randomDouble(2, 0, 10000);
                String tempGetOrderdescription = dataFaker.lorem().characters();

                Order newOrder = new Order(tempGetOrdamount, tempGetAdvanceamount, fakeCustomer, tempGetOrderdescription);

                newOrder.getPayments().add(pay1);
                fakeCustomer.getOrders().add(newOrder);
            }
